...
namespace SPOApp
{
    public static class BilSkadePortalGuide
    {
        

        

        public static List<BilSkadePortalGuideManualProperies> GetSourceFiles(ClientContext context, string sourceLibraryName)
        {
            ...
            List<BilSkadePortalGuideManualProperies> pages = new List<BilSkadePortalGuideManualProperies>();
            foreach (ListItem listItem in items)
            {
                ...
                if (listItem.FileSystemObjectType == FileSystemObjectType.File)
                {
                    ...
                    BilSkadePortalGuideManualProperies spp;
                    spp.WikiContent = (listItem["WikiField"] == null) ? "" : listItem["WikiField"].ToString();
                    spp.BilSkadePortalGuideCategory = (listItem["Kategori"] == null) ? "" : listItem["Kategori"].ToString();
        ...
        public static void CreateModernSitePages(ClientContext context, List<BilSkadePortalGuideManualProperies> pages)
        {

            //List<AnsvarManualProperies> sourcePages = pages;
            string targetContentTypeName = "BilskadePortalGuideManual";
            foreach (var p in pages)
            {
                CreatePages(context, p, targetContentTypeName);
                //if (p.FileName.Equals("Relationer.aspx"))
                //{
                //    CreatePages(context, p, targetContentTypeName);
                //}
        ...
        private static void CreatePages(ClientContext context, BilSkadePortalGuideManualProperies p, string targetContentTypeName)
        {
            ...
            if (!string.IsNullOrEmpty(p.BilSkadePortalGuideCategory))
            {
                SPOUtility.SetMetadataField(context, item, p.BilSkadePortalGuideCategory, "BilskadePortalGuideCategory");
                item.Update();
            }
        ...
        public static void CheckForLinks(ClientContext context) {
            ...
            {
                if (oListItem.ContentType.Name == "BilskadePortalGuideManual")
                {
