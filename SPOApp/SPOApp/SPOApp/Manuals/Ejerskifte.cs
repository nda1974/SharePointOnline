...
namespace SPOApp
{
    public static class Ejerskifte
    {
        

        

        public static List<EjerskifteManualProperies> GetSourceFiles(ClientContext context, string sourceLibraryName)
        {
            ...
            List<EjerskifteManualProperies> pages = new List<EjerskifteManualProperies>();
            foreach (ListItem listItem in items)
            {
                ...
                if (listItem.FileSystemObjectType == FileSystemObjectType.File)
                {
                    ...
                    EjerskifteManualProperies spp;
                    spp.WikiContent = (listItem["WikiField"] == null) ? "" : listItem["WikiField"].ToString();
                    spp.EjerskifteCategory= (listItem["Kategori"] == null) ? "" : listItem["Kategori"].ToString();
                    spp.EjerskifteArea= (listItem["Omr_x00e5_de"] == null) ? "" : listItem["Omr_x00e5_de"].ToString();
        ...
        public static void CreateModernSitePages(ClientContext context, List<EjerskifteManualProperies> pages)
        {

            //List<AnsvarManualProperies> sourcePages = pages;
            string targetContentTypeName = "EjerskifteManual";
        ...
        private static void CreatePages(ClientContext context, EjerskifteManualProperies p, string targetContentTypeName)
        {
            ...
            if (!string.IsNullOrEmpty(p.EjerskifteCategory))
            {
                SPOUtility.SetMetadataField(context, item, p.EjerskifteCategory, "EjerskifteCategory");
                item.Update();
            }
            if (!string.IsNullOrEmpty(p.EjerskifteArea))
            {
                SPOUtility.SetMetadataField(context, item, p.EjerskifteArea, "EjerskifteArea");
                item.Update();
            }
