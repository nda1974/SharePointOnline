...
var EventHooks = (function () {
    function EventHooks(eventHooksJson) {
        var _this = this;
        this._hooks = new Map();
        Object.getOwnPropertyNames(eventHooksJson).forEach(function (name) {
            var eventName = Event[name];
            if (eventName) {
                var foundHooks_1 = [];
                if (eventHooksJson[name]) {
                    eventHooksJson[name].forEach(function (hook) {
                        foundHooks_1.push(hook);
                    });
                }
                _this._hooks.set(eventName, foundHooks_1);
            }
        });
    }
    /**
     ...
     */
    EventHooks.prototype.get = function (event) {
