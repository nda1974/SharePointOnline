...
var MIN_BUNDLE_NAME_LENGTH = 6;
var MAX_BUNDLE_NAME_LENGTH = 64;
function migrateToLatestVersion(logWarning, configJson) {
    var config2SchemaPath = path.join(__dirname, './schemas/config.2.0.schema.json');
    var config2Schema = node_core_library_1.JsonFile.load(config2SchemaPath);
    var bundleNameRegexString = Object.keys(config2Schema.properties.bundles.patternProperties)[0];
    var bundleNameRegex = new RegExp(bundleNameRegexString);
...
function readConfigFileAndMigrate(logWarning, logVerbose, filename) {
    if (filename === void 0) { filename = index_1.defaultConfigPath; }
    var configJsonResult = index_1.readConfigFile(logVerbose, filename);
